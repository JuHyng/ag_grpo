...
def extract_all_code(test_cases: str) -> list[str]:
    if not test_cases.endswith("\n\n"):
        test_cases = test_cases + "\n\n"

    pattern = re.compile(r"(.*?)\n\n", re.DOTALL)
    matches = pattern.findall(test_cases)
    # DBG
    # print("matches: ", matches)
    extracted_answers_list = matches if len(matches) >= 1 else []
    return extracted_answers_list
...
def code_reward(
    ...
    **kwargs,
    ...
    breakpoint()
    test_cases = kwargs["test"]
    test_code_snippets = extract_all_code(test_cases[0]) # 0

    template = evaluation_script_template

    scripts = [
        # template.format(code=json.dumps(code), test_cases=json.dumps(json.dumps(info["test_cases"])))
        template.format(code=json.dumps(code), test_cases=json.dumps(test_code_snippets))
        # for code, info in zip(code_snippets, verification_info)
        for code in zip(code_snippets)
    ]
    # DBG
