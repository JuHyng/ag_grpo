...
class DiffuGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        self._metrics["train"]["max_front_logp"].append(self.accelerator.gather_for_metrics(front_per_token_logps.max()).max().item())
        self._metrics["train"]["max_back_logp"].append(self.accelerator.gather_for_metrics(back_per_token_logps.max()).max().item())
        ...
        if front_ref_per_token_logps is not None:
            ...
            self._metrics["train"]["max_front_ref_logp"].append(self.accelerator.gather_for_metrics(front_ref_per_token_logps.max()).max().item())
            self._metrics["train"]["max_back_ref_logp"].append(self.accelerator.gather_for_metrics(back_ref_per_token_logps.max()).max().item())
            # print min
