...
import wandb
from data_utils import extract_hash_answer
from math500_utils import remove_boxed, last_boxed_only_string, is_equiv, boxed_in_answer

from data_utils import strip_python_comments_and_docstrings
...
class OracleDiffuGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        completion_ids, completion_mask = inputs["completion_ids"], inputs["completion_mask"] # [num_completions, seq_len]
        ...
        self._metrics["train"]["mean_front_old_logp"].append(self.accelerator.gather_for_metrics(mean_front_old_logp).mean().item())
        self._metrics["train"]["mean_back_old_logp"].append(self.accelerator.gather_for_metrics(mean_back_old_logp).mean().item())
        self._metrics["train"]["min_front_logp"].append(self.accelerator.gather_for_metrics(front_per_token_logps.min()).min().item())
        self._metrics["train"]["min_back_logp"].append(self.accelerator.gather_for_metrics(back_per_token_logps.min()).min().item())
        ...
        if front_ref_per_token_logps is not None:
            ...
            self._metrics["train"]["min_front_ref_logp"].append(self.accelerator.gather_for_metrics(front_ref_per_token_logps.min()).min().item())
            self._metrics["train"]["min_back_ref_logp"].append(self.accelerator.gather_for_metrics(back_ref_per_token_logps.min()).min().item())
            # print min
            print(f"Step {self._step}: min_front_logp: {front_per_token_logps.min().item():.4f}, min_back_logp: {back_per_token_logps.min().item():.4f}, min_front_old_logp: {front_old_per_token_logps.min().item():.4f}, min_back_old_logp: {back_old_per_token_logps.min().item():.4f}, min_front_ref_logp: {front_ref_per_token_logps.min().item():.4f}, min_back_ref_logp: {back_ref_per_token_logps.min().item():.4f}")
    ...
    def generate(
        ...
        # with torch.amp.autocast_mode(enabled=self.args.fp16):
            bs = prompt.shape[0]
            dtype = model.dtype
            ...
            print(f"Generating {gen_length} tokens in {num_blocks} blocks of {block_length} tokens each, ")
            print(f"with {steps_per_block} diffusion steps per block (total {steps_per_block * num_blocks} steps).")
            
            if answer is not None:
                answer_length = answer.shape[1]   
                x[:, -answer_length:] = answer.clone()
                # num_blocks-=1 # last block is for answer, so reduce num_blocks by 1
            ...
            for num_block in range(num_blocks):
                ...
                # DBG
                # print(f"Block {num_block+1}/{num_blocks}: Masked tokens to fill: {block_mask_index} over {steps_per_block} steps")
                # print(num_transfer_tokens)
                if block_mask_index.sum() == 0:
                    # DBG
                    # print(f"Block {num_block+1}/{num_blocks}: No tokens to mask, skipping.")
                    continue
                ...
                for i in range(steps_per_block):
                    ...
                    if (x[:, start_idx:end_idx] == mask_id).sum() == 0:
                        print(f"early stop at block {num_block}, step {i} original step {steps_per_block} on {x.shape[0]} samples")
                        break
    ...
    def _get_per_token_logps(self, model, input_ids, logits_to_keep, mask_seeds):
        ...
        if self.args.loss_type == "default":
        ...
        elif self.args.loss_type == "mean_sequence":
            log_probs = F.softmax(completion_logits, dim=-1)
            per_token_logp = log_probs.gather(
                dim=-1, index=completion_targets.unsqueeze(-1)
            ).squeeze(-1)   
            
            log_mean_p = torch.log(torch.sum(per_token_logp, dim=1)) - torch.log(torch.tensor(logits_to_keep, device=per_token_logp.device, dtype=per_token_logp.dtype))  # [N]
            completion_log_probs = log_mean_p
            per_token_logps = completion_log_probs.view(num_iterations, batch_size, 1)
    ...
    def _generate_and_score_completions(
        ...
        additional_length = self.args.answer_additional_length or 0
        additional_text = "" + self.processing_class.pad_token * additional_length
        
        if self.args.dataset=="sudoku":
            solutions = [f"<answer>{x['solution']}</answer>{additional_text}" for x in inputs] 
            
        elif self.args.dataset=="countdown":
            solutions = [f"<answer>{x['target']}</answer>{additional_text}" for x in inputs]

        elif self.args.dataset=="gsm8k":
            solutions = [f"<answer>{x['answer']}</answer>{additional_text}" for x in inputs]

        elif self.args.dataset=="math":
            solutions = [f"<answer>{remove_boxed(last_boxed_only_string(x['answer']))}</answer>{additional_text}" for x in inputs]
        
        elif self.args.dataset=="kodcode":
            answer_length = self.processing_class(inputs[0]['solution'], return_tensors="pt", add_special_tokens=False)["input_ids"].size(1) + additional_length
            if answer_length > gen_length:
                print(f"Warning: answer_length {answer_length} > gen_length {gen_length}, truncating answer to fit.")
                solutions = [f"<answer>{strip_python_comments_and_docstrings(x['solution'])}</answer>{additional_text}" for x in inputs]
            else:
                solutions = [f"<answer>{x['solution']}</answer>{additional_text}" for x in inputs]
        else:
        ...
        # Get steps_per_block for answer generation
        assert gen_length % block_length == 0
        num_blocks = gen_length // block_length
        steps_per_block = max(1, steps // num_blocks)
        
        if self.args.no_answer_padding:
            # If no padding for answers, set answer_length to the actual length of each answer
            ...
            answer_lengths = [len(ids) for ids in solution_inputs["input_ids"]]
            max_answer_length = max(answer_lengths)
            answer_length = max_answer_length
            # answer_steps = max(1, steps // (gen_length // answer_length)) 
            if self.args.answer_steps:
                answer_steps = self.args.answer_steps
            else:
                answer_steps = max(1, round(steps * (answer_length / gen_length)))
            # DBG
            print(f"Using no_answer_padding: max_answer_length: {max_answer_length}, answer_steps: {answer_steps}")
        else:
            if self.args.answer_length:
                answer_length = self.args.answer_length
                # answer_steps = max(1, steps // (gen_length // answer_length)) 
                answer_steps = max(1, round(steps * (answer_length / gen_length)))
                # DBG
                # print(f"Using specified answer_length: {answer_length}, answer_steps: {answer_steps}")
                # result(diff_steps:32): Using specified answer_length: 32, answer_steps: 4
                
            elif num_blocks == 1:
                answer_length = gen_length // 8
                answer_steps = steps // 8
            else:
                answer_length = block_length
                answer_steps = steps_per_block
                
            solution_inputs = self.processing_class(
                text=solutions,
                return_tensors="pt",
                padding="max_length",
                max_length=answer_length,  # pad to the nearest block length
                padding_side="right",
                add_special_tokens=False,
            )
        ...
        with unwrap_model_for_generation(self.model_wrapped, self.accelerator) as unwrapped_model:
            ...
            # Process in batches
            # Front half: without answers
            prompt_completion_ids_all = []
            for i in range(0, num_base_generations, generation_batch_size):
                end_idx = min(i + generation_batch_size, num_base_generations)
            ...
            for i in range(num_base_generations, prompt_ids.size(0), generation_batch_size):
                end_idx = min(i + generation_batch_size, prompt_ids.size(0))
                batch_prompt_ids = prompt_ids[i:end_idx]
                batch_prompt_mask = prompt_mask[i:end_idx]
                batch_answer_ids = solution_ids[i:end_idx]
                batch_answer_mask = solution_mask[i:end_idx]
                
                print("" + "="*40)
                print("BEFORE generation, prompt:")
                print(self.processing_class.batch_decode(batch_prompt_ids[0], skip_special_tokens=False))
                ...
                batch_cot_completion_ids = self.generate(
                    ...
                    remasking=self.args.remasking,
                    mask_id=self.args.mask_id,
                    answer=batch_answer_ids,
                )
                
                # DEBUG
                print("\n\n"+ "="*40)
                print("!! Generated CoT with answer: !!")
                cot_texts = self.processing_class.batch_decode(batch_cot_completion_ids[0], skip_special_tokens=False)
                for cot_text in cot_texts:
                    print(cot_text)
                
                # Delete answer
                batch_cot_completion_ids = batch_cot_completion_ids[:, :-batch_answer_ids.size(1)]
                
                
                batch_prompt_completion_ids = self.generate(
                    model=unwrapped_model,
                    prompt=batch_cot_completion_ids,
                    steps=answer_steps,
                    gen_length=batch_answer_ids.size(1),
                    block_length=batch_answer_ids.size(1),
                    temperature=temperature,
                    cfg_scale=cfg_scale,
                    remasking=self.args.remasking,
                    mask_id=self.args.mask_id,
                ...
                print("\n\n"+ "="*40)
                print("After removing answer, continuing generation:")
                cot_texts = self.processing_class.batch_decode(batch_prompt_completion_ids[0], skip_special_tokens=False)
                for cot_text in cot_texts:
                    print(cot_text)
                ...
                del batch_prompt_ids, batch_prompt_mask, batch_answer_ids, batch_answer_mask, batch_prompt_completion_ids
                torch.cuda.empty_cache()
